...
export class RoyaltyToken extends ethereum.SmartContract {
  static bind(address: Address): RoyaltyToken {
    return new RoyaltyToken("RoyaltyToken", address);
  }
  ...
  storyIPId(): Bytes {
    let result = super.call("storyIPId", "storyIPId():(bytes32)", []);

    return result[0].toBytes();
  }

  try_storyIPId(): ethereum.CallResult<Bytes> {
    let result = super.tryCall("storyIPId", "storyIPId():(bytes32)", []);
    if (result.reverted) {
      return new ethereum.CallResult();
    }
    let value = result.value;
